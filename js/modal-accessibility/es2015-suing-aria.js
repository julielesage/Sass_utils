 ...
 */
import {isMobile} from "typescript/util/device-utils";
import {ScrollbarComponent} from "typescript/scrollbar/perfectScrollbarComponent";
...
(doc => {
    ...
    const MODAL_PREFIX_ID_ATTR = 'data-modal-prefix-id';
    const MODAL_CLASS_APPEND = 'data-modal-class-append';
    ...
    const createOverlay = config => {
      ...
      let id = config.modalPrefixId + MODAL_OVERLAY_ID;
      ...
      return `<span
                    data-modal-prefix-id="${config.modalPrefixId}"
                    id="${id}"
                    class="${overlayClass}"
    ...
    const createModal = config => {
      ...
      let id = config.modalPrefixId + MODAL_JS_ID;
      let btn_id = config.modalPrefixId + MODAL_BUTTON_JS_ID;
      let modal_content_id = config.modalPrefixId + MODAL_CONTENT_JS_ID;
      let modalClassName = config.modalPrefixClass + MODAL_CLASS_SUFFIX;
      let modalClassAppend = config.modalClassAppend;
      ...
      let title = config.modalTitle !== '' ?
        ...
        '';
      let button_close = `<button data-modal-prefix-id="${config.modalPrefixId}" type="button" class="${MODAL_BUTTON_JS_CLASS} ${buttonCloseClassName}" id="${btn_id}" title="${config.modalCloseTitle}" ${MODAL_BUTTON_CONTENT_BACK_ID}="${config.modalContentId}" ${MODAL_BUTTON_FOCUS_BACK_ID}="${config.modalFocusBackId}">
                               ${buttonCloseInner}
                              </button>`;
      ...
      if (content === '' && config.modalContentId) {
        let contentFromId = findById(config.modalContentId);
          if (contentFromId) {
              let formTransformation = contentFromId.innerHTML;
              if (contentFromId.querySelector('[data-js-form-transform]')) {
                  formTransformation = formTransformation.replace(/formtransform/g, "form");
              }
              content = `<div id="${modal_content_id}">
                              ${formTransformation}
                             </div`;
              // we remove content from its source to avoid id duplicates, etc.
              contentFromId.innerHTML = '';
              contentFromId.formTransformation = '';
          }

      }


      return `<dialog data-modal-prefix-id="${config.modalPrefixId}" id="${id}" class="${modalClassName} ${modalClassAppend}" ${ATTR_ROLE}="${MODAL_ROLE}" ${describedById} ${ATTR_OPEN} ${ATTR_LABELLEDBY}="${MODAL_TITLE_ID}">
    ...
    const closeModal = config => {
      ...
      if (config.contentBackId !== '') {
        ...
        if (contentBack) {
            ...
            let formTransformation = config.modalContent;
            if (formTransformation.includes("data-js-form-transform")) {
                formTransformation = formTransformation.replace("<form", "<formtransform");
                formTransformation = formTransformation.replace("</form", "</formtransform");
            }

          contentBack.innerHTML = formTransformation;
        }
      }
          ...
          doc.body
            .addEventListener(eventName, e => {
              ...
              if ((hasClass(e.target, MODAL_JS_CLASS) === true || parentModalLauncher !== '') && eventName === 'click') {
                let body = doc.querySelector('body');
                let htmlTag= doc.querySelector('html');
                ...
                let modalPrefixId = modalLauncher.hasAttribute(MODAL_PREFIX_ID_ATTR) === true ? modalLauncher.getAttribute(MODAL_PREFIX_ID_ATTR) + '-' : '';
                let modalClassAppend = modalLauncher.hasAttribute(MODAL_CLASS_APPEND) === true ? modalLauncher.getAttribute(MODAL_CLASS_APPEND) : '';
                ...
                body.insertAdjacentHTML('beforeEnd', createModal({
                  ...
                  modalPrefixId: modalPrefixId,
                  modalClassAppend: modalClassAppend,
                ...
                addClass(htmlTag, NO_SCROLL_CLASS);
                ...
                if (modalGiveFocusToId !== '') {
                  ...
                  if ( document.querySelector( '#quickview-description' ) && !isMobile()) {
                      new ScrollbarComponent( '#quickview-description' ).init();
                  }

                  if ( document.querySelector( '[data-js="modal__body-scroll"]' ) && !isMobile() ) {
                      new ScrollbarComponent( '[data-js="modal__body-scroll"]' ).init();
                  }
              ...
              let parentButton = searchParent(e.target, MODAL_BUTTON_JS_CLASS);
                ...
                let modalPrefixId = e.target.hasAttribute(MODAL_PREFIX_ID_ATTR) === true ? e.target.getAttribute(MODAL_PREFIX_ID_ATTR) : '';
                let id = modalPrefixId + MODAL_JS_ID;
                let btn_id = modalPrefixId + MODAL_BUTTON_JS_ID;
                let modal_content_id = modalPrefixId + MODAL_CONTENT_JS_ID;
                let modal_overlay_id = modalPrefixId + MODAL_OVERLAY_ID;

              if (
                (
                  e.target.getAttribute('id') === btn_id || parentButton !== '' ||
                ...
                let htmlTag= doc.querySelector('html');
                let wrapperBody = findById(WRAPPER_PAGE_JS);
                let modal = findById(id);
                let modalContent = findById(modal_content_id) ? findById(modal_content_id).innerHTML : '';
                let overlay = findById(modal_overlay_id);
                let modalButtonClose = findById(btn_id);
                ...
                if (!(e.target.getAttribute('id') === MODAL_OVERLAY_ID && backgroundEnabled === 'disabled')) {
                  ...
                  removeClass(htmlTag, NO_SCROLL_CLASS);
